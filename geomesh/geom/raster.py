...
class RasterGeom(BaseGeom):
    ...
    def get_multipolygon(self, zmin: float = None, zmax: float = None) -> MultiPolygon:
    ...
    def raster(self, raster: Union[Raster, str, os.PathLike]):
        ...
        if not isinstance(raster, Raster):
            raise TypeError(f'Argument raster must be of type {Raster}, {str} or {os.PathLike}, not type {type(raster)}.')
        self._raster = raster
